...
if __name__ == '__main__':
    ...
    # YOLO 모델 로드
    model = YOLO("models/Drone_Shadow_Detect_v4.pt")

    # 추론할 이미지가 있는 폴더 경로 지정
    image_folder = "Dataset/test/original/"  # 원하는 이미지 폴더 경로로 수정

    # 결과 이미지를 저장할 폴더 경로 지정
    output_folder = "Dataset/test/test_B/"  # 원하는 결과 이미지 폴더 경로로 수정

    # 원하는 이미지 사이즈로 설정
    img_size = 1280  # 원하는 이미지 사이즈로 수정

    # 리사이즈된 원본 이미지를 저장할 폴더 경로 지정
    resized_folder = "Dataset/test/test_A/"  # 새로운 폴더 경로로 수정

    # 이미지 폴더 내의 모든 이미지 파일에 대해 추론 및 마스크 이미지 저장
    for filename in os.listdir(image_folder):
        if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".JPG"):
            ...
            # 이미지 추론
            results = model.predict(image_path, imgsz=img_size)
            result = results[0]
            masks = result.masks
            ...
            # 결과 마스크 이미지 파일명 지정 (순차적으로 번호 붙임)
            mask_filename = f"{filename.split('.')[0]}.png"
            mask_output_path = os.path.join(output_folder, mask_filename)

            # 결과 마스크 이미지 저장
            combined_mask_img.save(mask_output_path, format="PNG")

            # 원본 이미지 리사이즈
            original_image = Image.open(image_path)
            original_image_resized = original_image.resize(combined_mask_img.size, Image.LANCZOS)

            # 리사이즈된 원본 이미지 파일명 지정 (순차적으로 번호 붙임)
            resized_filename = f"{filename.split('.')[0]}.png"
            resized_path = os.path.join(resized_folder, resized_filename)

            # 리사이즈된 원본 이미지 저장
            original_image_resized.save(resized_path, format="PNG")

    #-------------------------------------------------------------------------------------------

    parser = argparse.ArgumentParser(description='RGB denoising evaluation on the validation set of SIDD')
    parser.add_argument('--input_dir', default='Dataset/test/',
        type=str, help='Directory of validation images')
    parser.add_argument('--result_dir', default='results/',
        type=str, help='Directory for results')
    parser.add_argument('--weights', default='models/ISTD_plus_model_latest.pth',
